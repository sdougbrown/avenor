...
func TestThinkingEffortArguments(t *testing.T) {
	for _, level := range []string{"low", "medium", "high", "xhigh", "max"} {
		args := buildClaudeArgs("session", "server", runtime.StartOptions{Thinking: level})
		if got := strings.Join(args, " "); !strings.Contains(got, "--effort "+level) {
			t.Fatalf("args for %s = %q", level, got)
		}
	}
	if got := strings.Join(buildClaudeArgs("session", "server", runtime.StartOptions{}), " "); strings.Contains(got, "--effort") {
...
func TestThinkingEffortCapabilityMismatch(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "claude-channel") || !strings.Contains(err.Error(), "thinking") {
