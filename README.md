...
Avenor supports seven backends: `opencode-acp` (default), `opencode-http`, `codex-app-server`, `gemini-acp`, `cursor-acp`, `pi`, and `claude-channel` (experimental). For MCP-compatible clients, `avenor mcp` is the canonical Go-native MCP server — no Node/Bun required. The Node.js packages (`@dougbots/avenor-mcp`, `@dougbots/avenor-opencode`) remain for OpenCode integration. See [docs/mcp.md](docs/mcp.md) for MCP setup and tool details, or install the MCP server with bundled orchestration instructions via the [Claude Code plugin guide](docs/claude-plugin.md) or the [Codex plugin guide](docs/codex-plugin.md). Get your agent to check the docs to ensure it uses the CLI correctly. (Hilariously, this CLI is intended for your agent to use, not you as a human.)
